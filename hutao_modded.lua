...
combatS.hdtMoveSpeed = 5.6   -- LinearVelocity push speed (studs/s); was "Sprint Speed" (MoveTo era)
...
local function combatHDTBeginDrag(killerModel, blockTrack)
    pcall(function()
        ...
        -- LinearVelocity instance (replaces BodyVelocity / MoveTo)
        local lv = nil

        local function restoreMovement()
            -- Destroy LinearVelocity so physics is fully released
        ...
        -- Freeze sprint module so it doesn't fight us
        pcall(function()
            if sprintMod then
        ...
        -- Lock normal movement (keeps physics active for LinearVelocity)
        hum.WalkSpeed    = 0
        hum.JumpPower    = 0
        hum.PlatformStand = false

        -- Create an Attachment for the LinearVelocity
        local attachment = Instance.new("Attachment")
        attachment.Parent = hrp

        -- Create LinearVelocity — horizontal push only, gravity unaffected (Y maxforce = 0)
        lv = Instance.new("LinearVelocity")
        lv.Attachment0       = attachment
        lv.MaxForce          = Vector3.new(1e5, 0, 1e5)   -- no Y force → gravity stays natural
        ...
        local function stopMove()
            ...
            stopped = true
            -- clean up attachment too
            pcall(function() if attachment and attachment.Parent then attachment:Destroy() end end)
            restoreMovement()
        end

        -- Stop as soon as the block animation ends
        if blockTrack then
            blockTrack.Stopped:Connect(stopMove)
        end

        -- Drive LinearVelocity toward killer (FINAL AUTO BLOCK style, no MoveTo)
        task.spawn(function()
            ...
            while not stopped and tick() < deadline do
                ...
                task.wait()   -- run every frame for smooth push (same cadence as FINAL AB)
            end
...
sec_017:Slider({ Title = "Push Speed (studs/s)", Flag = "combatHDTMoveSpeed", Value = {Min=1,Max=60,Default=combatS.hdtMoveSpeed}, Step = 0.1, Callback=function(v) combatS.hdtMoveSpeed=v end
})
