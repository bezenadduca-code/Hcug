-- SAKIWARE | MM2 God Hub Edition | maintained by mitsuki
...
WindUI:Popup({
    ...
    Icon    = "sparkles",
    Content = CreateGradientText("Sakiware | MM2 God Hub Edition", Colors.Gold, Colors.LightBlue)
        .. "<br/>"
        .. CreateGradientText("by mitsuki", Colors.Blue, Colors.Purple),
    Buttons = {
        {
            Title    = "Exit",
            Callback = function() popupClosed = true end,
            Variant  = "Tertiary"
        },
        {
            Title    = "Copy Discord",
            Callback = function()
                ...
                WindUI:Notify({ Title = "Discord Copied!", Content = "Invite copied!", Icon = "check-circle", Duration = 3 })
                popupClosed = true
            end,
            Variant = "Secondary"
        },
...
svc.Run.RenderStepped:Connect(function()
    _frameCount += 1
    local now = tick()
    if now - _lastUpdate >= 1 then
        local fps = math.floor(_frameCount / (now - _lastUpdate))
        FPSTag:SetTitle("FPS: " .. fps)
        FPSTag:SetColor(fps >= 50 and Color3.fromRGB(0,255,0)
...
task.spawn(function()
    while true do
        ...
        if ok and ping then
            ...
            PingTag:SetColor(ping <= 50 and Color3.fromRGB(0,255,0)
                or ping <= 100 and Color3.fromRGB(255,200,0)
                or Color3.fromRGB(255,0,0))
        end
        task.wait(2)
    end
...
do
    ...
    function ChatLogger.createUI()
        ...
        pcall(function()
            ...
            chatScrollFrame.Name                    = "ChatScroller"
            chatScrollFrame.Size                    = UDim2.new(1,0,1,-70)
            chatScrollFrame.Position                = UDim2.new(0,0,0,32)
            ...
            chatContainer.Parent        = chatScrollFrame
    ...
    for _, ep in ipairs(endpoints or {}) do
        for _, n in ipairs(path) do
            if n.row == ep.row and n.col == ep.col then
                start = { row = ep.row, col = ep.col }; break
            end
        end
...
local function absGui()
    ...
    absScreenGui=Instance.new("ScreenGui"); absScreenGui.Name="AbsGui"; absScreenGui.ResetOnSpawn=false
    absScreenGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; absScreenGui.Parent=pg; return absScreenGui
end
local function absShowLabel(show)
    local g=absGui(); if not g then return end; local lbl=g:FindFirstChild("AbsTaunt")
    if not lbl then
        lbl=Instance.new("TextLabel"); lbl.Name="AbsTaunt"; lbl.Size=UDim2.new(0,500,0,50)
        lbl.Position=UDim2.new(0.5,-250,0.38,0); lbl.BackgroundTransparency=1
        lbl.TextColor3=Color3.new(1,1,1); lbl.TextStrokeTransparency=0.4; lbl.TextStrokeColor3=Color3.new(0,0,0)
        lbl.Text="At least they tried 😂"; lbl.Font=Enum.Font.GothamBold; lbl.TextSize=36
        lbl.TextTransparency=1; lbl.Parent=g
    end
...
local function absAddRing(model)
    ...
    pcall(function()
        local ring=Instance.new("Part"); ring.Name="AbsRing"; ring.Shape=Enum.PartType.Cylinder
        ring.Size=Vector3.new(0.1,abs.range*2,abs.range*2); ring.Color=Color3.fromRGB(220,50,50)
        ring.Material=Enum.Material.ForceField; ring.Transparency=0.5; ring.CanCollide=false
        ring.CanTouch=false; ring.CFrame=hrp.CFrame*CFrame.Angles(0,0,math.rad(90)); ring.Parent=hrp
...
local function absTrigger()
    ...
    local ch=lp.Character; local myRoot=ch and ch:FindFirstChild("HumanoidRootPart"); if not myRoot then return end
    local ttModel=absFindTwoTime(); if not ttModel then return end
    local ttRoot=ttModel:FindFirstChild("HumanoidRootPart"); if not ttRoot then return end
    ...
    task.spawn(function()
        ...
        while tick()<deadline do
            ...
            local ch2=lp.Character; local r2=ch2 and ch2:FindFirstChild("HumanoidRootPart")
...
local function absHookSounds()
    ...
    abs.soundConn=svc.WS.DescendantAdded:Connect(function(obj)
        ...
        local id=obj.SoundId:match("%d+"); if id and absTriggerSounds[id] then absTrigger() end
    end)
end
...
local function absStop()
    ...
    absCleanRings(); abs.locked=false; absShowLabel(false)
end
...
espAttach = function(obj, tag, color, isChar)
    ...
    pcall(function()
        local hl=Instance.new("Highlight"); hl.Name=tag; hl.FillColor=color; hl.FillTransparency=0.8
        ...
        local bb=Instance.new("BillboardGui"); bb.Name=tag.."_bb"; bb.Adornee=root
        bb.Size=UDim2.new(0,100,0,20); bb.StudsOffset=Vector3.new(0,isChar and 3.5 or 3.8,0)
        ...
        lbl.TextColor3=color; lbl.TextStrokeTransparency=0.5; lbl.TextStrokeColor3=Color3.new(0,0,0)
        lbl.TextSize=15; lbl.FontFace=Font.new("rbxasset://fonts/families/AccanthisADFStd.json"); lbl.Parent=bb
        if isChar then
...
local function addHighlight(obj,color,tag,label,offset)
    ...
    hl.Name=tag.."_HL"; hl.FillColor=color; hl.FillTransparency=mset.transparency; hl.OutlineColor=color
    hl.OutlineTransparency=0.1; hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop; hl.Adornee=obj; hl.Parent=obj
    if root then
        local bb=Instance.new("BillboardGui"); bb.Name=tag.."_BB"; bb.Adornee=root
        bb.Size=UDim2.new(0,130,0,24); bb.StudsOffset=Vector3.new(0,offset or 3,0); bb.AlwaysOnTop=true; bb.Parent=obj
        local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(1,0,1,0); lbl.BackgroundTransparency=1
        lbl.Text=label; lbl.TextColor3=color; lbl.TextStrokeColor3=Color3.new(0,0,0); lbl.TextStrokeTransparency=0.2
...
local function addPuddleHighlight(part,color,tag,label)
    ...
    local hl=Instance.new("Highlight")
    hl.Name=tag.."_HL"; hl.FillColor=color; hl.FillTransparency=mset.transparency; hl.OutlineColor=color
    hl.OutlineTransparency=0.1; hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop; hl.Adornee=part; hl.Parent=part
    task.wait(0.05)
    ...
    local blackCircle=Instance.new("CylinderHandleAdornment"); blackCircle.Name="PuddleBlack"; blackCircle.Adornee=holder
    blackCircle.Color3=Color3.fromRGB(0,0,0); blackCircle.Transparency=0.2; blackCircle.Radius=radius
    blackCircle.Height=0.02; blackCircle.CFrame=CFrame.Angles(math.rad(90),0,0); blackCircle.ZIndex=5
    blackCircle.AlwaysOnTop=true; blackCircle.Parent=holder
    local redOutline=Instance.new("CylinderHandleAdornment"); redOutline.Name="PuddleRed"; redOutline.Adornee=holder
    redOutline.Color3=Color3.fromRGB(255,0,0); redOutline.Transparency=0.4; redOutline.Radius=radius+0.8
    redOutline.Height=0.02; redOutline.CFrame=CFrame.Angles(math.rad(90),0,0); redOutline.ZIndex=4
    redOutline.AlwaysOnTop=true; redOutline.Parent=holder
    ...
    local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(1,0,1,0); lbl.BackgroundTransparency=1
    lbl.Text=label; lbl.TextColor3=Color3.fromRGB(255,255,255); lbl.TextStrokeColor3=Color3.fromRGB(255,0,0)
    lbl.TextStrokeTransparency=0.1; lbl.TextSize=11; lbl.Font=Enum.Font.GothamBold; lbl.Parent=bb
...
do
    ...
    task.spawn(function()
        ...
        if ok and re then
            local oldNC; oldNC=hookmetamethod(game,"__namecall",function(self,...) local method=getnamecallmethod(); local args={...}
                if method=="FireServer" and self==re then if args[1]=="UseActorAbility" and args[2] and args[2][1] then
                    local ok2,bs=pcall(function() return buffer.tostring(args[2][1]) end)
                    if ok2 and bs and string.find(bs,"ThrowPizza") then elliotIsThrowing=true; elliotThrowTS=tick() end end end
                return oldNC(self,...) end)
        end
    end)
    ...
    local function elliotFindTarget()
        ...
        for _,s in ipairs(sf:GetChildren()) do
            if s~=lp.Character then local h=s:FindFirstChildOfClass("Humanoid"); local r=s:FindFirstChild("HumanoidRootPart")
                if h and r and h.Health>0 then local val=elliotTargetMode=="Closest" and (r.Position-elliotHRP.Position).Magnitude or h.Health; if val<bestVal then best=r; bestVal=val end end
            end
        end
        return best
    end
    local function elliotAimAt(tgt)
        ...
        local vel=tgt.AssemblyLinearVelocity; local pos=tgt.Position; local predPos=pos+(tgt.CFrame.LookVector*2)
    ...
    local function elliotArcCalc(startPos,lookVec)
        ...
        for i=0,elliotArcSegs do
            ...
            if i>0 then local d=pos-last; local dm=d.Magnitude
                if dm>0 then local res=svc.WS:Raycast(last,d.Unit*dm,rp); if res then table.insert(pts,res.Position); break end end
            end
    ...
    local function elliotUpdateArc()
        ...
        local pts=elliotArcCalc(startPos,elliotHRP.CFrame.LookVector); elliotClearArc()
        ...
        for i,p in ipairs(pts) do
            ...
            part.Position=p; part.Anchored=true; part.CanCollide=false; part.Material=Enum.Material.Neon; part.Shape=Enum.PartType.Ball
...
do
    ...
    local chanceHoldingKey=false; local chanceAiming=false; local chanceStartTime=0; local chanceAimDuration=1.7
    ...
    local function chanceGetPing()
        ...
        local raw=(ok and stat or 100)/1000; table.insert(chancePingSamples,raw); if #chancePingSamples>5 then table.remove(chancePingSamples,1) end
    ...
    local function chancePredict(hrp)
        ...
        if chanceAntiBait then
            ...
            if model and chanceKillerSpeeds[model.Name] then
                local maxSpd=chanceKillerSpeeds[model.Name].run+2; if speed>maxSpd then vel=vel.Unit*maxSpd; speed=maxSpd end
            end
        end
    ...
    local function chanceHookAnimator(char)
        ...
        anim.AnimationPlayed:Connect(function(track)
            ...
            local id=track.Animation.AnimationId:match("%d+"); if id and chanceTriggers[id] then
    ...
    svc.Run.RenderStepped:Connect(function()
        ...
        if not chanceBodyGyro or not chanceBodyGyro.Parent then
            chanceBodyGyro=Instance.new("BodyGyro"); chanceBodyGyro.MaxTorque=Vector3.new(0,math.huge,0)
            chanceBodyGyro.P=10000; chanceBodyGyro.D=500; chanceBodyGyro.Parent=chanceHRP
        end
...
do
    ...
    sec_vee_spray:Toggle({ Title="Enable Fast Spray", Type="Checkbox", Flag="veeeFastSpray", Default=vee_fastSpray, Callback=function(on) vee_fastSpray=on; if not on then sprayPhase=0; sprayBaseCF=nil end end })
end
...
do
    ...
    local function jd_axeDoLock()
        ...
        jd_axeLocked=true; local savedAutoRotate=hum.AutoRotate; hum.AutoRotate=false; local deadline=tick()+jd_AXE_LOCK_DURATION
        ...
        local oldNC; oldNC=hookmetamethod(game,"__namecall",function(self,...)
            ...
            if method=="FireServer" and self==re then
                ...
                if jd_enabled and eventName==(jd_lp.Name.."CrystalInput") then
                    if not jd_holdActive then jd_holdActive=true
                        task.spawn(function()
                            local deadline=tick()+jd_HOLD_DURATION
                            while tick()<deadline and jd_enabled and not jd_unloaded do
                                if jd_RemoteEvent then local b=buffer.create(8); buffer.writeu32(b,0,2); buffer.writef32(b,4,svc.WS.DistributedGameTime); jd_RemoteEvent:FireServer(jd_lp.Name.."CrystalInput",{b}) end
                                task.wait(1/30)
                            end; jd_holdActive=false
                        end)
                    end
                end
    ...
    sec_024:Slider({ Title="Hold Duration",   Flag="jdHoldDur",    Value={Min=0.3,Max=2.0,Default=jd_HOLD_DURATION},Step=0.1, Callback=function(v) jd_HOLD_DURATION=v end })
    ...
    sec_026:Button({ Title="Unload Jane Doe", Callback=function() if jd_unloaded then return end; jd_unloaded=true; jd_enabled=false; jd_aimbotOn=false; jd_axeEnabled=false; pcall(jd_removePatch) end})
end
...
do
    ...
    local plasma_oldCB=nil; local plasma_rf=nil
    local function plasmaGetNearestKiller()
        ...
        for _,model in ipairs(kf:GetChildren()) do
            if model~=char then local hrp=model:FindFirstChild("HumanoidRootPart"); local hum=model:FindFirstChildOfClass("Humanoid")
                if hrp and hum and hum.Health>0 then local d=(hrp.Position-myHRP.Position).Magnitude; if d<bestDist then bestDist=d; best=hrp end end end
        end
        return best
    end
    ...
    local function plasmaGetVelocity(hrp)
        ...
        local dt=now-data.lastTime; if dt>0 then data.vel=(pos-data.lastPos)/dt; data.lastPos=pos; data.lastTime=now end; return data.vel
    end
    local function plasmaPatchRF()
        ...
        if not ok or not rf then return end; plasma_rf=rf; plasma_oldCB=getcallbackvalue(rf,"OnClientInvoke")
        rf.OnClientInvoke=function(reqName,data,...) if reqName=="GetMousePosition" and plasma_enabled then local hrp=plasmaGetNearestKiller(); if hrp then local vel=plasmaGetVelocity(hrp); local predPos=hrp.Position+vel*plasma_prediction+Vector3.new(0,plasma_aimOffset,0); return predPos end end; if plasma_oldCB then return plasma_oldCB(reqName,data,...) end end
    end
    ...
    task.spawn(plasmaPatchRF); lp.CharacterAdded:Connect(function() plasma_motionData={} end)
...
do
    ...
    local nova_enabled=false; local nova_aimOffset=0.0; local nova_prediction=0.12; local nova_oldCB=nil; local nova_rf=nil; local nova_motionData={}
    ...
    local function novaPatch()
        ...
        local ok,rf=pcall(function() return svc.RS:WaitForChild("Modules",10):WaitForChild("Network",10):WaitForChild("Network",10):WaitForChild("RemoteFunction",10) end)
        if not ok or not rf then return end; nova_rf=rf; nova_oldCB=getcallbackvalue(rf,"OnClientInvoke")
        rf.OnClientInvoke=function(reqName,data,...) if reqName=="GetMousePosition" and nova_enabled then local hrp=novaGetNearestSurvivor(); if hrp then local vel=novaGetVelocity(hrp); local predPos=hrp.Position+vel*nova_prediction+Vector3.new(0,nova_aimOffset,0); return predPos end end; if nova_oldCB then return nova_oldCB(reqName,data,...) end end
    end
...
local function aiGetNearest()
    ...
    for _,m in ipairs(sf:GetChildren()) do local h=m:FindFirstChildOfClass("Humanoid"); local r=m:FindFirstChild("HumanoidRootPart")
        if h and r and h.Health>0 then local d=(aiPredictPosition(r,aiCfg.predScale)-myHRP.Position).Magnitude; if d<dist then dist=d; best={model=m,humanoid=h,root=r} end end end
    return best
end
local function aiGetMyKillerName() local ch=lp.Character; if not ch then return nil end; local kf=getTeamFolder("Killers"); if not kf then return nil end; if not ch:IsDescendantOf(kf) then return nil end; return ch.Name end
...
local function aiStartMove()
    ...
    aiState.moveConn=svc.Run.Heartbeat:Connect(function()
        ...
        if aiState.wpIndex<=#aiState.waypoints then local wp=aiState.waypoints[aiState.wpIndex]; if (hrp.Position-wp.Position).Magnitude<5 then aiState.wpIndex+=1 end; hum:MoveTo(wp.Position); return end
        aiState.lastMovePos=aiState.lastMovePos or hrp.Position; local targetPos=aiPredictPosition(targetHRP,aiCfg.predScale)
        aiState.lastMovePos=aiState.lastMovePos:Lerp(targetPos,0.15); hum:MoveTo(aiState.lastMovePos)
    end)
end
local function aiKillerLoop()
    ...
    while ai_enabled do
        ...
        if not aiState.target then continue end; local targetHRP=aiState.target.root
        local dist=(targetHRP.Position-hrp.Position).Magnitude
        if dist<=aiCfg.slashRange and tick()-aiState.lastSlash>aiCfg.slashCooldown then aiFireSlash(); aiState.lastSlash=tick() end
        if tick()-aiState.lastPath>aiCfg.pathInterval then
            aiState.lastPath=tick(); local path=PathfindingService:CreatePath({AgentRadius=2,AgentHeight=5,AgentCanJump=true})
...
secAIMain:Toggle({ Title="Enable Killer AI Farm", Type="Checkbox", Flag="aiKillerEnabled", Default=ai_enabled,
    Callback=function(on)
        ai_enabled=on
        if on then
            if ai_thread then task.cancel(ai_thread) end
            aiState.target=nil; aiState.waypoints={}; aiState.wpIndex=1; aiState.lastMovePos=nil
...
local combatS = {
    autoBlockOn=false, blockType="Block", detectionRange=18, blockDelay=0,
    doubleBlock=true, antiBait=false, abMissChance=0, autoPunchOn=false,
    hdtEnabled=false, hdtSpeed=12, hdtDelay=0, hdtMissChance=0,
    killerCircles=false, facingCheck=true, facingVisual=false, facingVisRadius=3,
    aimPunchActive=false, punchPrediction=2.3, aimPunchDuration=0.5,
}

local TRIGGER_SOUNDS={
    ["140242176732868"]=true,["136323728355613"]=true,["115026634746636"]=true,["84116622032112"]=true,
    ["108907358619313"]=true,["127793641088496"]=true,["86174610237192"]=true,["95079963655241"]=true,
    ["101199185291628"]=true,["119942598489800"]=true,["84307400688050"]=true,["105200830849301"]=true,
    ["75330693422988"]=true,["82221759983649"]=true,["81702359653578"]=true,["85853080745515"]=true,
    ...
    ["136728245733659"]=true,["107444859834748"]=true,["76959687420003"]=true,["72425554233832"]=true,
    ["96594507550917"]=true,["139996647355899"]=true,["107345261604889"]=true,["127557531826290"]=true,
    ["108651070773439"]=true,["74842815979546"]=true,["124397369810639"]=true,["76467993976301"]=true,
    ["118493324723683"]=true,["78298577002481"]=true,["116527305931161"]=true,["5148302439"]=true,
    ["98675142200448"]=true,["128367348686124"]=true,["71805956520207"]=true,["125213046326879"]=true,
    ["103684883268194"]=true,["109246041199659"]=true,["80540530406270"]=true,["139523195429581"]=true,
    ["105204810054381"]=true,["114742322778642"]=true,["116468089135195"]=true,["112809109188560"]=true,
...
local function combatIsFacing(myRoot,targetRoot,killerName)
    ...
    if bait then
        ...
        local side=math.abs(vel:Dot(targetRoot.CFrame.RightVector)); if side>3 then return false end; return dot>STRICT_FACING_DOT+0.05
    end
...
local combatPunchAiming=false; local combatPunchLastTriggerTime=0; local combatOriginalAutoRotate=nil; local combatOriginalHRPRotY=nil; local combatOriginalHRPCFrame=nil; local combatAimConnection=nil
...
local function combatSetupAimPunch(char)
    ...
    combatAimConnection=anim.AnimationPlayed:Connect(function(track)
        ...
        if combatS.aimPunchActive and combatTrackedPunchAnimations[animId] then
            ...
            if h and r and not combatPunchAiming then combatOriginalAutoRotate=h.AutoRotate; combatOriginalHRPCFrame=r.CFrame; combatOriginalHRPRotY=select(2,r.CFrame:ToEulerAnglesYXZ()) end
...
svc.Run.RenderStepped:Connect(function()
    if not combatS.aimPunchActive then
        if combatPunchAiming then
            ...
            if hrp and combatOriginalHRPCFrame~=nil then hrp.CFrame=CFrame.new(hrp.Position)*combatOriginalHRPCFrame.Rotation; hrp.AssemblyAngularVelocity=Vector3.zero; combatOriginalHRPCFrame=nil; combatOriginalHRPRotY=nil end
    ...
    if elapsed>combatS.aimPunchDuration then
        ...
        if hrp and combatOriginalHRPCFrame~=nil then hrp.CFrame=CFrame.new(hrp.Position)*combatOriginalHRPCFrame.Rotation; hrp.AssemblyAngularVelocity=Vector3.zero; combatOriginalHRPCFrame=nil; combatOriginalHRPRotY=nil end
        if hum then hum.AutoRotate=combatOriginalAutoRotate~=nil and combatOriginalAutoRotate or true; combatOriginalAutoRotate=nil end; return
    end
...
local function combatTryBlockFromSound(sound,preId)
    ...
    if soundPart then local model=soundPart:FindFirstAncestorOfClass("Model"); if model and model:FindFirstChildOfClass("Humanoid") then local kf=combatGetKillersFolder(); if kf and model:IsDescendantOf(kf) then killerModel=model end end end
    ...
    if combatS.antiBait then
        ...
        local toUs=myRoot.Position-hrp.Position; if toUs.Magnitude>0.1 then if vel:Dot(toUs.Unit)<-3 then return end end
        if dist>13 then return end; if dist>6 then local sideSpeed=math.abs(vel:Dot(hrp.CFrame.RightVector)); if sideSpeed>6 and vel:Dot(toUs.Unit)<0 then return end end
    end
...
local function combatHookSound(sound)
    ...
    combatSoundHooks[sound]={playedConn,propConn,destroyConn}; if sound.IsPlaying then task.spawn(combatTryBlockFromSound,sound,preId) end
end
...
local function combatHDTBeginDrag(killerModel)
    ...
    local targetHRP=combatGetKillerHRP(killerModel); if not targetHRP then return end
    if combatRollMiss(combatS.hdtMissChance) then return end
    _combatHDTDebounce=true; local oldWalk=hum.WalkSpeed; local oldJump=hum.JumpPower; local oldPlatformStand=hum.PlatformStand
...
local function combatOnBlockAnim(track)
    pcall(function()
        ...
        if combatS.hdtEnabled and not _combatHDTDebounce then local now=tick(); if now-combatHDTLastTime>=HDT_CD then combatHDTLastTime=now; local nearest=combatGetNearestKiller(); if nearest then task.spawn(function() if combatS.hdtDelay>0 then task.wait(combatS.hdtDelay) end; combatHDTBeginDrag(nearest) end) end end end
...
local function combatUpdateCircles()
    local kf=combatGetKillersFolder(); if not kf then return end
    for _,k in pairs(kf:GetChildren()) do local hrp=k:FindFirstChild("HumanoidRootPart"); if hrp then
...
local function combatStartLoops()
    ...
    combatSoundTickConn=svc.Run.Heartbeat:Connect(function() if not combatS.autoBlockOn then return end; local now=tick(); for sound,t in pairs(combatSoundBlockedUntil) do if now>t then combatSoundBlockedUntil[sound]=nil end end end)
...
-- AB Indicator Dot
local _abDotGui=nil; local _abDotBtn=nil; local _abDotDragging,_abDotDragStart,_abDotDragPos=false,nil,nil
local _settingsAbToggle=nil; local _g1337AbToggle=nil
...
local function abDotCreate()
    ...
    local pg=lp:FindFirstChildOfClass("PlayerGui"); if not pg then return end
    _abDotGui=Instance.new("ScreenGui"); _abDotGui.Name="ABDotGui"; _abDotGui.ResetOnSpawn=false
    _abDotGui.DisplayOrder=20; _abDotGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; _abDotGui.Parent=pg
    _abDotBtn=Instance.new("TextButton"); _abDotBtn.Size=UDim2.new(0,52,0,52); _abDotBtn.Position=UDim2.new(1,-64,0.5,-26)
    _abDotBtn.AnchorPoint=Vector2.new(0,0); _abDotBtn.BackgroundColor3=Color3.fromRGB(200,40,40)
    _abDotBtn.BackgroundTransparency=0; _abDotBtn.BorderSizePixel=0; _abDotBtn.Text="ab"
    _abDotBtn.TextColor3=Color3.fromRGB(255,255,255); _abDotBtn.TextSize=13; _abDotBtn.Font=Enum.Font.GothamBold
    ...
    _abDotBtn.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
            _abMouseHeld=true; _abDotDragging=false; _abDotDragStart=inp.Position; _abDotDragPos=_abDotBtn.Position
        end
    end)
    _abDotBtn.InputEnded:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
            _abMouseHeld=false; _abDotDragStart=nil; task.delay(0.05,function() _abDotDragging=false end)
        end
    end)
...
lp.CharacterAdded:Connect(function(char)
    task.wait(0.6); combatRefreshAnimator(); combatSetupAimPunch(char)
    if combatS.autoBlockOn then combatSetupSoundWatcher() end
    if combatS.autoBlockOn or combatS.killerCircles or combatS.facingVisual then combatStartLoops() end
end)
...
_g1337AbToggle=sec_015:Toggle({ Title="Auto Block (Audio)", Flag="combatAutoBlock", Default=combatS.autoBlockOn,
    Callback=function(on) combatS.autoBlockOn=on; abDotSetState(on); if on then combatSetupSoundWatcher(); combatStartLoops() else combatStopLoops() end end, Type="Checkbox"})
...
sec_015:Slider({  Title="Block Miss %",      Flag="combatMissChance", Value={Min=0,Max=100,Default=combatS.abMissChance}, Step=1, Callback=function(v) combatS.abMissChance=v end })
...
sec_017:Slider({ Title="HDT Miss %",     Flag="combatHDTMiss",  Value={Min=0,Max=100,Default=combatS.hdtMissChance}, Step=1, Callback=function(v) combatS.hdtMissChance=v end })
...
sec_018:Slider({ Title="Facing Visual Size",Flag="combatFacingSize",  Value={Min=1,Max=10,Default=combatS.facingVisRadius}, Step=0.5, Callback=function(v) combatS.facingVisRadius=v; for _,vis in pairs(combatFacingVisuals) do if vis and vis.Parent then vis.Radius=v; local adornee=vis.Adornee; if adornee then vis.CFrame=CFrame.new(0,-(adornee.Size.Y/2+0.04),-v)*CFrame.Angles(math.rad(90),0,0) end end end end })

local sec_019=tabGuest1337:Section({ Title="Aim Punch Lock", Opened=true })
sec_019:Toggle({ Title="Aim Punch",       Flag="combatAimPunch",   Default=combatS.aimPunchActive, Callback=function(on) combatS.aimPunchActive=on; if on and lp.Character then combatSetupAimPunch(lp.Character) end; if not on and combatAimConnection then combatAimConnection:Disconnect(); combatAimConnection=nil end end, Type="Checkbox"})
...
------------------------------------------------------------------------
-- SETTINGS TAB (interface / chat / anti-taph)
------------------------------------------------------------------------
