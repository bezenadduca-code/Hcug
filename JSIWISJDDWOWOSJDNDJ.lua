...
local function combatHDTBeginDrag(killerModel)
    ...
    local sm = speedModule()
    local oldWalk = hum.WalkSpeed

    if sm then
        if not sm.DefaultsSet then pcall(function() sm.Init() end) end
        pcall(function() sm.SprintSpeed = combatS.hdtSpeed end)
        pcall(function() sm:Toggle(true) end)
    else
        hum.WalkSpeed = combatS.hdtSpeed
    end
    ...
    conn = svc.Run.Heartbeat:Connect(function()
        if not _combatHDTDebounce then
            conn:Disconnect()
            if sm then
                pcall(function() sm:Toggle(false) end)
                pcall(function() sm.SprintSpeed = 26 end)
            else
        ...
        if (targetHRP.Position - hrp.Position).Magnitude <= 2.0 then
    ...
    task.spawn(function()
        hum.AutoRotate = false
        local sw = tick()
        while _combatHDTDebounce and tick() - sw < 0.4 do
            pcall(function()
                local nk = combatGetNearestKiller()
                if nk and hrp and hrp.Parent then
                    local tHRP2 = combatGetKillerHRP(nk)
                    if tHRP2 then
                        hrp.CFrame = CFrame.new(hrp.Position, Vector3.new(tHRP2.Position.X, hrp.Position.Y, tHRP2.Position.Z))
                    end
                end
            end)
            task.wait()
