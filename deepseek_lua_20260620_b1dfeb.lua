-- Noli Dev Panel + Nova Proximity + Collision Blocker (Hooked)
...
local NovaNetwork = ReplicatedStorage.Modules.Network.Network.RemoteEvent
...
local NOVA_RADIUS     = NoliConfig.NovaExplosionRadius  -- 26 studs
local NOVA_WINDUP     = NoliConfig.NovaWindup
...
------------------------------------------------------------------------
-- [NEW] Collision Blocker using __namecall hook
------------------------------------------------------------------------

local collisionKey = playerName .. "VoidRushCollision"
local oldNamecall = nil

local function applyCollisionBlocker()
    ...
    oldNamecall = hookmetamethod(RemoteEvent, "__namecall", function(self, ...)
        ...
        if method == "FireServer" then
            ...
            -- If this is the specific collision event, DROP it completely.
            if name == collisionKey then
                return nil
            end
        end
        
        -- Let everything else pass safely
        return oldNamecall(self, ...)
    end)
end

local function removeCollisionBlocker()
    if not oldNamecall then return end
    
    -- We cannot easily "unhook" a hook. We have to let it pass all traffic and destroy the hook reference.
    -- Instead, we force the toggle's logic to actively check if it should block or let pass.
    -- The most robust method is to override the local variable inside the hook via a closure.
    
    -- Cleaner way: We define the hook UP FRONT but make it check a boolean state.
    oldNamecall = nil 
end

-- REDEFINED hook to check the `state.noWallCollision` boolean dynamically
local RemoteEvent = ReplicatedStorage:WaitForChild("Modules"):WaitForChild("Network"):WaitForChild("Network"):WaitForChild("RemoteEvent")

local originalNamecall = hookmetamethod(RemoteEvent, "__namecall", function(self, ...)
    -- Always allow other remotes
    if self ~= RemoteEvent then
    ...
    if method == "FireServer" then
        ...
        -- Check if the event is the collision key, AND check our state
    ...
    trackConn = RunService.Heartbeat:Connect(function()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            if dist <= detonateRadius and not fired then
                fired = true
                trackConn:Disconnect()
                NovaNetwork:FireServer("UseActorAbility", {
...
local function startNovaListener()
    novaListener = workspace.DescendantAdded:Connect(function(inst)
        if inst.Name == "Voidstar" and inst:IsA("BasePart") then
            trackVoidstar(inst)
        end
    end)
end
...
btnWC.MouseButton1Click:Connect(function()
    ...
    if state.noWallCollision then
        print("[NoliPanel] Collision Blocker ENABLED.")
    else
